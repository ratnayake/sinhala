...
- Ship as an **MSIX package** (Windows 10 1809+/11 native format): gives clean install/uninstall, auto-start registration via the package manifest's startup task extension, and a Microsoft Store-compatible path if you ever want one, without touching `HKLM`.
- Target **.NET 8** self-contained or framework-dependent (framework-dependent + the near-universal .NET runtime presence on Win10/11 keeps the package small; self-contained trades size for zero-install-dependency simplicity — pick self-contained for a consumer tool that shouldn't require "install .NET first").
- Code-sign the package (even a self-signed cert for internal use, a real cert for public distribution) — unsigned MSIX won't install by double-click on a clean machine.
